...
// @name           IITC Plugin: Hesal Tracker
// @category       Layer
...
// PLUGIN START ////////////////////////////////////////////////////////
window.PLAYER_TRACKER_MAX_TIME = 30*60*60*1000; // in milliseconds
window.PLAYER_TRACKER_MIN_ZOOM = 7;
window.PLAYER_TRACKER_MIN_OPACITY = 0.3;
window.PLAYER_TRACKER_LINE_COLOR_HESAL = '#07AEFF';
window.PLAYER_TRACKER_LINE_COLOR_RES = '#0066FF';
window.PLAYER_TRACKER_LINE_COLOR_ENL = '#00FF00';



// use own namespace for plugin
window.plugin.playerTracker = function() {};
window.plugin.playerTracker.labelLayers = {};
window.plugin.playerTracker.labelLayerGroup = null;
window.plugin.playerTracker.NAME_WIDTH = 120;
window.plugin.playerTracker.NAME_HEIGHT = 30;
...
window.plugin.playerTracker.setup = function() {
  ...
  plugin.playerTracker.drawnTracesHesal = new L.LayerGroup();
  plugin.playerTracker.drawnTracesRes = new L.LayerGroup();
  plugin.playerTracker.drawnTracesEnl = new L.LayerGroup();

  // to avoid any favouritism, we'll put the player's own faction layer first
    window.addLayerGroup('Player Tracker Hesal', plugin.playerTracker.drawnTracesHesal, true);
  ...
  window.plugin.playerTracker.labelLayerGroup = new L.LayerGroup();
  window.addLayerGroup('Player Tracker Names', window.plugin.playerTracker.labelLayerGroup, true);

  map.on('layeradd',function(obj) {
    if(obj.layer === plugin.playerTracker.drawnTracesHesal || obj.layer === plugin.playerTracker.drawnTracesRes || obj.layer === plugin.playerTracker.drawnTracesEnl) {
...
window.plugin.playerTracker.closeIconTooltips = function() {
    plugin.playerTracker.drawnTracesHesal.eachLayer(function(layer) {
...
window.plugin.playerTracker.removeLabel = function(guid) {
  var previousLayer = window.plugin.playerTracker.labelLayers[guid];
  if(previousLayer) {
    window.plugin.playerTracker.labelLayerGroup.removeLayer(previousLayer);
    delete plugin.playerTracker.labelLayers[guid];
  }
}
window.plugin.playerTracker.clearAllPortalLabels = function() {
  for (var guid in window.plugin.playerTracker.labelLayers) {
    window.plugin.playerTracker.removeLabel(guid);
  }
}
...
window.plugin.playerTracker.drawData = function() {
  ...
  window.plugin.playerTracker.clearAllPortalLabels();

  var polyLineByAgeHesal = [[], [], [], []];
  ...
  $.each(plugin.playerTracker.stored, function(plrname, playerData) {
    ...
    for(var i = 1; i < playerData.events.length; i++) {
      ...
      if(playerData.team == 'RESISTANCE' && playerData.nick != 'Hesal'){
          polyLineByAgeRes[ageBucket].push(line);
      }
      if(playerData.team == 'ENLIGHTENED' && playerData.nick != 'Hesal'){
           polyLineByAgeEnl[ageBucket].push(line);
      }
      if(playerData.nick == 'Hesal'){
          polyLineByAgeHesal[ageBucket].push(line);
      }
    ...
    // marker itself
    var icon = playerData.team === 'RESISTANCE' || playerData.nick === 'Hesal'?  new plugin.playerTracker.iconRes() :  new plugin.playerTracker.iconEnl();
    ...
    var label = L.marker(gllfe(last), {
      icon: L.divIcon({
        className: 'plugin-player-tracker-names',
        iconAnchor: [window.plugin.playerTracker.NAME_WIDTH/2,63],
        iconSize: [window.plugin.playerTracker.NAME_WIDTH,window.plugin.playerTracker.NAME_HEIGHT],
        html: plrname
      }),
      guid: guid,
    });
    window.plugin.playerTracker.labelLayers[guid] = label;

    label.addTo(window.plugin.playerTracker.labelLayerGroup);
    ...
    if (playerData.team == 'RESISTANCE' && playerData.nick != 'Hesal'){
        m.addTo(plugin.playerTracker.drawnTracesRes)
    }
    if (playerData.team == 'ENLIGHTENED'&& playerData.nick != 'Hesal'){
        m.addTo(plugin.playerTracker.drawnTracesEnl)
    }

    if (playerData.nick == 'Hesal'){
        m.addTo(plugin.playerTracker.drawnTracesHesal)
    }
  ...
  $.each(polyLineByAgeHesal, function(i, polyLine) {
    if(polyLine.length === 0) return true;
    var opts = {
      weight: 4-0.25*i,
      color: PLAYER_TRACKER_LINE_COLOR_HESAL,
      clickable: false,
      opacity: 1-0.2*i,
      dashArray: "5,6"
    };
    $.each(polyLine, function(ind,poly) {
      L.polyline(poly, opts).addTo(plugin.playerTracker.drawnTracesHesal);
    });
  });
    ...
    var opts = {
      weight: 3-0.25*i,
      color: PLAYER_TRACKER_LINE_COLOR_RES,
      clickable: false,
      opacity: 1-0.3*i,
      dashArray: "5,6"
    };
  ...
  $.each(polyLineByAgeEnl, function(i, polyLine) {
    ...
    var opts = {
      weight: 3-0.25*i,
      color: PLAYER_TRACKER_LINE_COLOR_ENL,
      clickable: false,
      opacity: 1-0.3*i,
      dashArray: "5,6"
    };
